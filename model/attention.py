...
from utils.graph import mask_adjs, mask_x
from torch_geometric.nn import GCNConv, GCN, DenseGCNConv
...
class Attention(torch.nn.Module):
    ...
    def forward(self, x, adj, attention_mask=None):
        ...
        Q = torch.cat(Q.split(dim_split, 2), 0)
        K = torch.cat(K.split(dim_split, 2), 0)

        if attention_mask is not None:
            attention_mask = torch.cat([attention_mask for _ in range(self.num_heads)], 0)
            attention_score = Q.bmm(K.transpose(1,2))/math.sqrt(self.out_dim)
            A = self.activation(attention_mask + attention_score )
        else:
            A = self.activation(Q.bmm(K.transpose(1,2))/ math.sqrt(self.out_dim) ) # (B x num_heads) x N x N
...
#         # edge_index, edge_weight = dense_to_sparse(adj, flags)
        
#         if self.conv == 'GCN':
#             # Q = self.gnn_q(x, edge_index, edge_weight)
#             # K = self.gnn_k(x, edge_index, edge_weight) 
#             Q = self.gnn_q(x, adj)
#             K = self.gnn_k(x, adj) 
#         else:
#             Q = self.gnn_q(x) 
#             K = self.gnn_k(x)
#         # V = self.gnn_v(x, edge_index, edge_weight)
#         V = self.gnn_v(x, adj)
#         dim_split = self.attn_dim // self.num_heads
#         Q = torch.cat(Q.split(dim_split, 2), 0)
#         K = torch.cat(K.split(dim_split, 2), 0)

#         B_multiply_H, N, _ = Q.size()
#         A = torch.zeros(B_multiply_H, N, N, device=Q.device)
...
#             Q_block = Q[:, i:end_i, :]
#             # K_block = K[:, :, j:end_j]
#             attention_score = Q_block.bmm(K.transpose(1, 2)) / math.sqrt(self.out_dim)
            
#             A[:, i:end_i, :] = self.activation(attention_score)
...
#             # gnn_q = GCNConv(in_dim, attn_dim)
#             # gnn_k = GCNConv(in_dim, attn_dim)
#             # gnn_v = GCNConv(in_dim, out_dim)
