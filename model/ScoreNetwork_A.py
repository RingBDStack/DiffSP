...
from utils.graph import mask_adjs, pow_tensor, mask_x, node_feature_to_matrix
...
class ScoreNetworkA(BaselineNetwork):
    ...
    def __init__(self, max_feat_num, nhid, num_layers, num_linears, 
        ...
        super(ScoreNetworkA, self).__init__(max_feat_num, nhid, num_layers, num_linears, 
                                            c_init, c_hid, c_final, adim, num_heads=4, conv='GCN')
    ...
    def forward(self, x, adj, flags):
        ...
        # self.mask = self.mask.to(score.device)
        # mask = torch.ones([score.size(-2), score.size(-1)]) - torch.eye(score.size(-1))
        # mask = mask.unsqueeze_(0).to(score.device)
        # score = score * mask
        score = score * (1 - torch.eye(score.size(-1), device=score.device)).unsqueeze(0)
